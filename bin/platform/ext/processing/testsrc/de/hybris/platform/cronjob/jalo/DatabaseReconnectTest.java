 ...
 */
@IntegrationTest
public class DatabaseReconnectTest extends HybrisJUnit4Test
{
	...
	public void testReconnectDuringCronjob() throws InterruptedException
	{
		if (hasJUnitJDBCSetup())
		{
			...
			try
			{
				...
				TestUtils.disableFileAnalyzer("Connections set to fail", 10000);
		...
		{
			LOG.warn(
					"Cannot run DatabaseReconnectTest.testReconnectDuringCronjob() since current tenant doesnt have JUnit JDBC settings!");
		}
	}
	...
	static class ReconnectPerformable implements TestJob.Performable
	{
		...
		public CronJobResult perform(final CronJob cronJob)
		{
			SearchResult rs = FlexibleSearch.getInstance().search(
					"SELECT {PK} FROM {Country} WHERE {" + Item.CREATION_TIME + "}<?now",
