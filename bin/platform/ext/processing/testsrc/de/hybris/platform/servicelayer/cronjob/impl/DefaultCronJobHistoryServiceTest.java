...
public class DefaultCronJobHistoryServiceTest extends AbstractCronJobHistoryTest
{
	...
	public void testGetCronJobHistoryByRange()
	{
		...
		// when
		final List<CronJobHistoryModel> results = cronJobHistoryService.getCronJobHistoryBy(null, null, baseStartDate,
				finishDate);
	...
	public void testGetCronJobHistoryByJobCode()
	{
		...
		final List<CronJobHistoryModel> historyForJobOne = getCronJobHistoryModelList().stream()
		                                                                               .filter(h -> StringUtils.equals(
				                                                                               h.getJobCode(), jobOne.getCode()))
	...
	public void testGetCronJobHistoryByUserAndStartAndFinishDate()
	{
		...
		// when
		final List<CronJobHistoryModel> results = cronJobHistoryService.getCronJobHistoryBy(adminUser, null, startDate,
				finishDate);
	...
	{
		//given
		final CronJobModel cronJob = cronJobs.get(0);
		// when
		final Long result = cronJobHistoryService.getAverageExecutionTime(cronJob, TimeUnit.SECONDS);

		// then
		Assertions.assertThat(result).isEqualTo(14400);
	}
