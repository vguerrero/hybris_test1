...
@IntegrationTest
public class ItemCopyCreatorTest extends ServicelayerTest
{
	...
	public void testExceptionDuringCreationOtherAttributes() throws JaloBusinessException
	{
		...
		final ItemCopyCreator copyCreator = new ItemCopyCreator(ctx, null, source, target, blackList, whiteList,
				Collections.singletonMap(Product.CODE,
						UUID.randomUUID().toString()));

		final Product result = copyCreator.copy();

		Assert.assertNotNull("Result should be not null ",
				result);//TODO when tx feature activated any reference fail should prevent from creating the item
...
//		Assert.assertSame(pCopy, pCopy2);
	...
	// HORST-4519 (add @Test and change to public)
	private void testExceptionDuringCreationPartOfAttributes() throws JaloPriceFactoryException
	{
