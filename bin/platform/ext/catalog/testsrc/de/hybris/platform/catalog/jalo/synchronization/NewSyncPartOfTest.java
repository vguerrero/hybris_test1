		...
		if (!success.equals(cronJob.getResult()))
		{
			...
			try
			{
				while (reader.readNextLine())
				{
					...
					System.err.println(
							"remaining item to sync: src=" + schedule.getSrcPK() + "/" + pkToString(
									schedule.getSrcPK()) + " tgt=" + schedule.getTgtPK() + "/" + pkToString(schedule.getTgtPK())
									+ " ts=" + schedule.getTimestampPK() + "/" + pkToString(
									schedule.getTimestampPK()) + " pending=" + schedule.getPendingAttributes());
				}
			}
			finally
			{
