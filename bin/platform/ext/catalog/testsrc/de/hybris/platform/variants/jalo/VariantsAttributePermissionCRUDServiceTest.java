...
public class VariantsAttributePermissionCRUDServiceTest extends ServicelayerBaseTest
{
	...
	private void setUpVariantAttribute()
	{
		...
		final VariantAttributeDescriptor varAttributeJalo = VariantsManager.getInstance()
		                                                                   .createVariantAttributeDescriptor(params);
		params = new HashMap<>();
	...
	private void testAttributeDescriptor(final String permissionName, final AttributeCheckHelper checkHelper,
	                                     final ComposedTypeModel type, final ComposedTypeModel subType,
	                                     ...
	                                     final AttributeDescriptorModel subTypeAttribute)
	{
		...
		Assert.assertFalse("Current user should not be granted \"" + permissionName + "\" permission to " + subTypeAttribute
						+ " attribute",
				checkHelper.isGranted(subTypeAttribute.getEnclosingType().getCode(), subTypeAttribute.getQualifier()));
		 ...
		 */
		permissionManagementService.addAttributePermission(subTypeAttribute,
				new PermissionAssignment(permissionName, currentUser));
