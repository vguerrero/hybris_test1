...
public class IdTokenHelperTest
{
	...
	public void buildAndVerify()
	{
		...
		try
		{
			idTokenHelper = new IdTokenHelper.IdTokenBuilder(
					new IdTokenHelper.HeaderBuilder().alg(algorithm).kid(KID).getHeaders(),
					new IdTokenHelper.ClaimsBuilder().iss(ISS)
					                                 .nonce(NONCE)
					                                 .state(STATE)
					                                 .sub(SUB)
					                                 .iat()
					                                 .exp(idTokenValiditySeconds)
					                                 .addScopes(SCOPE_NAME, SCOPE)
					                                 .getClaims()).build();
	...
	public void addScopes()
	{
		...
		idTokenHelper = new IdTokenHelper.IdTokenBuilder(new IdTokenHelper.HeaderBuilder().getHeaders(),
				new IdTokenHelper.ClaimsBuilder().addScopes(SCOPE_NAME, Stream.of("a")
				                                                              .collect(
						                                                              Collectors
								                                                              .toList()))
				                                 .addScopes(SCOPE_NAME, Stream.of("b")
				                                                              .collect(
						                                                              Collectors
								                                                              .toList()))
				                                 .getClaims());
