...
public class Europe1TargetPriceDiscountsTest extends ServicelayerBaseTest
{
	...
	public void setUp() throws Exception
	{
		...
		modelService.saveAll(cat, catVer, product, currency, discountP1, discountP2, discountP3, order, entry, unit, basePrice,
				discountGroup);
	}
	...
	{
		/**
		 * see https://jira.hybris.com/browse/ECP-398
		 * <li>discountP1 is relative, which doesn't have currency attached, so its priority is lower than discountP2 and
		 * discountP3.</li>
		 * <li>So after order recalculation, discountP2 will be applied first, then discountP3, and discountP1 will be the
		 * last one.</li>
		 */
		...
		assertEquals(Double.valueOf("200.0").doubleValue(), entry.getDiscountValues().get(0).getAppliedValue(), 0.00001);
		assertTrue(entry.getDiscountValues().get(0).isAsTargetPrice());
		assertTrue(entry.getDiscountValues().get(0).isAbsolute());

		assertEquals(Double.valueOf("99.90").doubleValue(), entry.getDiscountValues().get(1).getAppliedValue(), 0.00001);
		assertFalse(entry.getDiscountValues().get(1).isAsTargetPrice());
		assertTrue(entry.getDiscountValues().get(1).isAbsolute());

		assertEquals(Double.valueOf("70.0").doubleValue(), entry.getDiscountValues().get(2).getAppliedValue(), 0.00001);
		assertFalse(entry.getDiscountValues().get(2).isAsTargetPrice());
		assertFalse(entry.getDiscountValues().get(2).isAbsolute());

		assertEquals(Double.valueOf("630.00"), entry.getTotalPrice());
		assertEquals(Double.valueOf("630.00"), order.getTotalPrice());
	}
