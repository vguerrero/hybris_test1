...
public class PriceRowFilterTest extends ServicelayerBaseTest
{
	...
	public void shouldGiveAway()
	{
		...
		final PriceValueInfoCriteria params = createPriceRowParams(1, pRowModel.getUnit(), pRowModel.getCurrency(), null,
				isGiveaway);
		final Collection<PriceRowModel> prmList = defaultPriceRowValueFilter.filter(Collections.singletonList(pRowModel), params);
		assertThat(prmList).isEmpty();
	}
	...
	public void shouldFilterOutRowWithWrongDateRange()
	{
		...
		final PriceValueInfoCriteria params = createPriceRowParams(1, pRowModel.getUnit(), pRowModel.getCurrency(), beforeDate,
				false);
		final Collection<PriceRowModel> prmList = defaultPriceRowValueFilter.filter(Collections.singletonList(pRowModel), params);
		assertThat(prmList).isEmpty();
	}

	PriceValueInfoCriteria createPriceRowParams(final long _quantity, final UnitModel unit, final CurrencyModel curr,
	                                            final Date date,
