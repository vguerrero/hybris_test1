		...
		sessionService.executeInLocalViewWithParams(ImmutableMap.of(Item.DISABLE_ITEMCHECK_BEFORE_REMOVABLE, Boolean.TRUE),
				new SessionExecutionBody()
				{
					...
					public void executeWithoutResult()
					{
						...
						final CatalogModel defaultCatalog = modelService.create(
								CatalogModel.class);
						defaultCatalog.setId(asUUID());
						modelService.saveAll();

						final CatalogVersionModel catalogVersion = modelService.create(
								CatalogVersionModel.class);
						catalogVersion.setCatalog(defaultCatalog);
						catalogVersion.setVersion(asUUID());
						catalogVersion.setActive(Boolean.TRUE);

						PersistenceTestUtils.saveAndVerifyThatPersistedThroughSld(
								modelService, catalogVersion);
						PersistenceTestUtils.verifyThatUnderlyingPersistenceObjectIsSld(
								catalogVersion);
