...
public class SQLRecoverableExceptionHandlerDemoTest
{
	...
	private JFrame showDemoFrame()
	{
		...
		final Color[] scale = new Color[]
				{ Color.GREEN, Color.GREEN.darker(), Color.GREEN.darker().darker(), Color.GREEN.darker()
				                                                                               .darker().darker(), Color.RED };
	...
	static class TestThread extends Thread
	{
		...
		private static void doTheTest()
		{
			...
			while (true)
			{
				try
				{
					fs.search("select {PK} from {Principal} where {creationtime} < ?now",
