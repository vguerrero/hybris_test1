...
import de.hybris.platform.jalo.Item.ItemImpl;
...
public class JaloItemWrappingTest extends HybrisJUnit4Test
{
	...
	public void testItemWrappingOptimization() throws ConsistencyCheckException
	{
		...
		final ItemImpl itf = i.getImplementation();
	...
	public void testItemWrappingOptimizationMultiThreaded() throws ConsistencyCheckException
	{
		...
		final long newTimeMultThreaded = testWrappingPerformanceMultiThreaded(THREADS, COUNT, ImplType.NEW,
				TIMEOUT_SECONDS_PER_RUN);
	...
	{
		final de.hybris.platform.test.RunnerCreator<TestRunner> creator = new de.hybris.platform.test.RunnerCreator<TestRunner>()
		{
			@Override
		...
		final TestThreadsHolder<TestRunner> holder = new TestThreadsHolder<>(threads, creator, true);
	...
	static class TestRunner implements Runnable
	{
		...
		TestRunner(final ItemImpl itf, final int count, final ImplType impl)
		{
			this.itf = itf;
			this.count = count;
			this.impl = impl;
	...
	private static long testWrappingPerformanceSingleThreaded(final ItemImpl itf, final int count, final ImplType impl)
	{
