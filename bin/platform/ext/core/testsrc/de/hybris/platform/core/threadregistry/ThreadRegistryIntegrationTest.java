...
public class ThreadRegistryIntegrationTest extends ServicelayerBaseTest
{
	...
	public void shouldSuccessfullyRenamePoolableThread() throws Exception
	{
		...
		try (final ThreadPool pool = new ThreadPool("junit", 5))
		{
			...
			poolableThread.execute(new NamedRunnable()
			{
				...
				{
					namedThreadName.set(
							(String) testSupportForDefaultRegistry().getAttributeFromCurrentOperation(
									StandardAttributes.THREAD_NAME));
					testFinished.countDown();
				}
	...
	public void shouldSuccesfullyRegisterInitThread() throws InterruptedException
	{
		...
		final Thread t = new RegistrableThread()
		{
			...
			{
				final OperationInfo operationInfo = OperationInfo.builder().asNotSuspendableOperation()
				                                                 .withCategory(OperationInfo.Category.INIT_OR_UPDATE).build();
				OperationInfo.updateThread(operationInfo);

				//then
				assertThat(defaultThreadRegistry().getStatus()).isEqualTo(SystemStatus.WAITING_FOR_UPDATE);

				final OperationInfo operationInfo2 = OperationInfo.builder().asNotSuspendableOperation()
				                                                  .withCategory(OperationInfo.Category.WEB_REQUEST).build();
				OperationInfo.updateThread(operationInfo2);

				assertThat(defaultThreadRegistry().getStatus()).isEqualTo(SystemStatus.RUNNING);
				latch.countDown();
			}
		};
		...
		final Thread t = new RegistrableThread()
		{
			...
			protected void internalRun()
			{
				try
				{
					...
					assertThat(testSupportForDefaultRegistry().getAttributeFromCurrentOperation("foo")).isNotNull()
					                                                                                   .isEqualTo("bar");
