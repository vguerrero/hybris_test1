 ...
 */
@IntegrationTest
public class DefaultPaginatedFlexibleSearchServiceIntegrationTest extends ServicelayerTransactionalBaseTest
{
	...
	public void shouldNotFailWithOverTheLimitPages()
	{
		...
		//then
		assertPaginationResults(0, 6, 1, pageSize, currentPage, 3, Boolean.FALSE, Boolean.TRUE, searchResult);
	}
	...
	public void shouldNotFailWithOverTheLimitPagesAndNoTotal()
	{
		...
		final int pageSize = 2;
		final int currentPage = 10;
		final boolean needsTotal = false;
		...
		//then
		assertPaginationResults(0, 0, 1, pageSize, currentPage, 0, Boolean.FALSE, Boolean.TRUE, searchResult);
	}
	...
	public void shouldNotFailWithNoPagesAndNoTotal()
	{
		...
		//then
		assertPaginationResults(0, 0, 1, pageSize, 0, 0, Boolean.FALSE, Boolean.FALSE, searchResult);
	}
	...
	public void shouldSearchWithoutTotal()
	{
		...
		//then
		assertPaginationResults(2, 2, 1, pageSize, currentPage, 1, Boolean.FALSE, Boolean.TRUE, searchResult);
	}
	...
	public void shouldSearchWithoutTotalFirstPage()
	{
		...
		//then
		assertPaginationResults(2, 2, 1, pageSize, currentPage, 1, Boolean.TRUE, Boolean.FALSE, searchResult);
	}
	...
	public void shouldSearchWithTotalFirstPage()
	{
		...
		//then
		assertPaginationResults(2, 6, 1, pageSize, currentPage, 3, Boolean.TRUE, Boolean.FALSE, searchResult);
	}
	...
	public void shouldSearchWithTotalLastPage()
	{
		...
		//then
		assertPaginationResults(2, 6, 1, pageSize, currentPage, 2, Boolean.FALSE, Boolean.TRUE, searchResult);
	}
	 ...
	 *
	 * @param expectedPaginatedResultSize the expected result count
	 * @param expectedTotalResultSize     the expected total results of the search results
	 * @param expectedSortSize            the expected sort list size
	...
	protected void assertPaginationResults(final int expectedPaginatedResultSize, final int expectedTotalResultSize,
	                                       final int expectedSortSize, final int expectedPageSize, final int expectedCurrentPage,
	                                       final int expectedNumberOfPages,
	                                       final Boolean expectedHasNextValue, final Boolean expectedHasPreviousValve,
	                                       final SearchPageData<UserGroupModel> searchResult)
	{
		assertPaginationResults(expectedPaginatedResultSize, expectedTotalResultSize, expectedSortSize, searchResult);

		final PaginationData pagination = searchResult.getPagination();
		Assert.assertEquals("Unexpected page size", expectedPageSize, pagination.getPageSize());
		Assert.assertEquals("Unexpected current page", expectedCurrentPage, pagination.getCurrentPage());
		Assert.assertEquals("Unexpected number of pages", expectedNumberOfPages, pagination.getNumberOfPages());
		Assert.assertEquals("Unexpected hasNext value", expectedHasNextValue, pagination.getHasNext());
		Assert.assertEquals("Unexpected hasPrevious value", expectedHasPreviousValve, pagination.getHasPrevious());
	}
	 ...
	 *
	 * @param id   the id to be set as Uid in the model
	 * @param name the name to be set in the model
	 */
