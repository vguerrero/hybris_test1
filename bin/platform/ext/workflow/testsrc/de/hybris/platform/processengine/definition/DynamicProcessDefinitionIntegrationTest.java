...
public class DynamicProcessDefinitionIntegrationTest extends ServicelayerBaseTest
{
			...
			TimeoutException
	{
		...
		waitFor(businessProcess, INITIAL_NUMBER_OF_NODES);
	...
	public void businessProcessShouldPickUpLatestProcessDefinition() throws InterruptedException, TimeoutException
	{
		...
		final BusinessProcessModel businessProcess = businessProcessService.startProcess(TEST_PROCESS_CODE, TEST_DEFINITION_CODE);
		waitFor(businessProcess, UPDATED_NUMBER_OF_NODES);
			...
			TimeoutException
	{
		...
		final BusinessProcessModel businessProcess = businessProcessService.createProcess(TEST_PROCESS_CODE,
	...
	private void waitFor(final BusinessProcessModel model, final int numberOfBusinessProcessActions) throws TimeoutException
	{
		final long maxWaitTime = System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(numberOfBusinessProcessActions);
