...
public class DefaultPersistenceService implements PersistenceService
{
	...
	@Override
	public void deleteItem(final ItemLookupRequest lookupRequest) throws EdmException
	{
		final ItemModel item = lookupItem(lookupRequest);
		getModelService().remove(item);
	}

	private ItemModel lookupItem(final ItemLookupRequest lookupRequest) throws EdmException
	{
	...
	private ODataEntry toODataEntry(final ItemLookupRequest lookupRequest, final ConversionOptions options, final ItemModel item)
	{
		...
		{
			LOG.error("An exception occurred while attempting to retrieve the ODataEntry with an ItemConversionRequest", e);
