...
public class DefaultODataProcessor extends ODataSingleProcessor
{
			...
			final String requestContentType, final String responseContentType) throws ODataException
	{
		...
		{
			LOG.error("Exception occurred while parsing the request body. Failed to create entity of type {}", entityType.getName(), e);
		...
		catch (final PersistenceRuntimeApplicationException e)
		{
			LOG.error("", e);
			throw e;
		}
			...
			throws ODataException
	{
		...
		{
			LOG.error("Exception while parsing the batch request body.", e);
			throw new ODataPayloadProcessingException(e);
		}
	...
	private ODataResponse read(final UriInfo uriInfo, final String contentType) throws ODataException
	{
		try
		{
			...
			return getoDataResponseBuilder(readerResponse)
				...
				LanguageNotSupportedException | InvalidLookupDataException | BatchLimitExceededException ex)
		{
			LOG.error("", ex);
			throw ex;
		}
