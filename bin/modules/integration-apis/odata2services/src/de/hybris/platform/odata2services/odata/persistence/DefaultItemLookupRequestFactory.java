...
public class DefaultItemLookupRequestFactory implements ItemLookupRequestFactory
{
	...
	public ItemLookupRequest create(final UriInfo uriInfo, final ODataContext context, final String contentType)
	{
		...
		{
			LOGGER.warn("Failed to create a look up request", e);
	...
	public ItemLookupRequest create(final ODataContext context, final EdmEntitySet edmEntitySet, final ODataEntry oDataEntry, final String integrationKey) throws EdmException
	{
		...
		final IntegrationItem item = convert(context, edmEntitySet, oDataEntry);
	...
	public ItemLookupRequest create(final DeleteUriInfo uriInfo, final ODataContext context)
	{
		...
		try
		{
			return builder
					.withAcceptLocale(locale)
					.withIntegrationObject(getServiceNameFromContext(context))
					.withODataEntry(getIntegrationKeyToODataEntryGenerator().generate(entitySet, getIntegrationKey(entitySet, keyPredicates)))
					.withEntitySet(entitySet).build();
		}
			...
			final String contentType) throws ODataException
	{
		...
		final IntegrationItem item = convert(context, entitySet, calculatedODataEntry);
		return builder
	...
	private IntegrationItem convert(final ODataContext context, final EdmEntitySet entitySet, final ODataEntry oDataEntry) throws EdmException
	{
		return getEntryConverter().convert(context, entitySet, oDataEntry);
	}
