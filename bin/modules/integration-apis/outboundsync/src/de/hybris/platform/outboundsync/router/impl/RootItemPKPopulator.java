...
public class RootItemPKPopulator
{
	...
	{
		if (!outboundItem.getIntegrationObject().getRootItemType().isPresent())
		{
	...
	{
		LOG.debug("Populating root item PK for root item DTO {}", itemDto);
	...
	private List<OutboundItemDTO> deriveRootItems(final OutboundItemDTO dto, final ItemModel item, final Collection<Object> parentItems)
	{
		final List<OutboundItemDTO> dtos = parentItems.stream()
				.peek(itemModel -> LOG.debug("Populated root item PK for item DTO with reference to root: {} -> {}", dto, itemModel))
