...
{
	private static final Logger LOGGER = LoggerFactory.getLogger(GettableChangesCollectorProvider.class);
	...
	protected Class getCollectorClass(final String collectorClassName)
	{
		try
		{
			...
			{
				LOGGER.warn("{} is not of GettableChangesCollector type", collectorClassName);
			}
		}
		catch (final ClassNotFoundException e)
		{
			LOGGER.error("An exception occurred trying to create class " + collectorClassName, e);
		}
		return null;
	}
	...
	protected GettableChangesCollector createCollector(final Class clazz)
	{
		...
		{
			LOGGER.error("An exception occurred while creating a new instance of " + clazz.getName(), e);
		}
	...
	{
		LOGGER.info("Using the default changes collector");
